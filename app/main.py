...
async def health_check(mcp_conn_manager: MCPConnectionManager = Depends(get_mcp_connection_manager)):
	mcp_manager_status = "unavailable"
	...
	if mcp_conn_manager:
		...
		for server_id in mcp_conn_manager.server_configs.keys():
			...
			if details:
				mcp_server_statuses[server_id] = {"status": details.get("status"),
												  "error": details.get("error_message"),
												  "ui_ready": details.get("ui_layout_retrieved", False)}
			else:
				mcp_server_statuses[server_id] = {"status": "unknown", "error": "State missing"}
	if hasattr(app.state, 'project_views_service') and app.state.project_views_service: pvs_status = "initialized"
	return {
		"status": "ok", "timestamp": datetime.now().isoformat(), "service": settings.APP_NAME,
		"version": app.version, "mcp_manager_status": mcp_manager_status,
		"project_views_service_status": pvs_status, "mcp_server_connections": mcp_server_statuses
	}
...
async def websocket_endpoint(
	...
	try:
		...
		if not mcp_server_id:
			logger.error(f"[WS/{ws_stream_id}] Missing MCP server ID.")
			disconnect_reason = "Missing target server ID"
			await websocket.close(code=1008, reason=disconnect_reason)
			return
		logger.info(f"[WS/{ws_stream_id}] Target MCP Server ID: '{mcp_server_id}'")
		session_key_for_cleanup = mcp_server_id

		# --- Register WebSocket with ConnectionManager ---
		...
		# --- Handshake: Receive Client Capabilities ---
		...
		try:
			...
			if 'WebSocketLogger' in locals() and hasattr(WebSocketLogger, 'log_text_received'):
				WebSocketLogger.log_text_received(websocket, ws_stream_id, first_message_text)
			else:
				logger.debug(f"[WS/{ws_stream_id}] RECV HANDSHAKE TEXT: {first_message_text[:100]}...")
			...
			if handshake_data.get("type") == "register_capabilities":
				...
				if isinstance(supported_primitives, list):
					...
					logger.info(
						f"[WS/{ws_stream_id}] Handshake OK. Client capabilities: {supported_primitives}")
				else:
			...
			logger.error(f"[WS/{ws_stream_id}] Handshake error: {handshake_err}",
						 exc_info=True)
			disconnect_reason = "Handshake error"
			raise WebSocketDisconnect(code=1011,
									  reason=disconnect_reason)

		# --- Get Pre-established MCP Session ---
		...
		if mcp_error or not mcp_session:
			...
			try:
				await websocket.send_text(json.dumps({"type": "error", "payload": {
					"message": f"Target service '{mcp_server_id}' not available: {mcp_error}"}}))
			except Exception as send_err:
				logger.error(f"[WS/{ws_stream_id}] Error sending MCP session error: {send_err}")
			disconnect_reason = f"MCP Session Error: {mcp_error}"
		...
		# --- Generate Initial UI State & Send Tool Schemas ---
		logger.info(f"[WS/{ws_stream_id}] Generating initial UI state and sending tool schemas...")
		try:
			root_element: Optional[UIElement] = await views_service.get_project_ui_hierarchy(websocket=websocket,
			...
			if root_element:
				...
				if 'WebSocketLogger' in locals() and hasattr(WebSocketLogger, 'log_text_sent'):
					WebSocketLogger.log_text_sent(ws_stream_id, initial_state_json)
				else:
					logger.debug(f"[WS/{ws_stream_id}] SENT Initial UI State.")
				...
				if discovered_tools:
					...
					for tool_name, tool_data in discovered_tools.items():
						if not isinstance(tool_data, dict):
							logger.warning(f"[{ws_stream_id}] Skipping invalid tool data for '{tool_name}'.")
							continue
						try:
							...
							logger.error(
								f"[{ws_stream_id}] Failed to validate/create ToolSchemaInfo for '{tool_name}': {schema_err}")
						except Exception as parse_err:
							logger.error(
								f"[{ws_stream_id}] Error processing tool schema data for '{tool_name}': {parse_err}")
					if tool_schemas_for_payload:
						...
						try:
							...
							if 'WebSocketLogger' in locals() and hasattr(WebSocketLogger, 'log_text_sent'):
								...
								logger.debug(f"[WS/{ws_stream_id}] SENT Tool Schemas.")
							logger.info(
								f"[{ws_stream_id}] Successfully sent tool schemas for {len(tool_schemas_for_payload)} tools.")
						except Exception as send_schema_err:
							logger.error(
								f"[{ws_stream_id}] Failed to serialize or send tool schemas: {send_schema_err}",
								exc_info=True)
					else:
						logger.warning(
							f"[{ws_stream_id}] No valid tool schemas processed to send for server '{mcp_server_id}'.")
				else:
					logger.warning(
						f"[{ws_stream_id}] No discovered tools found for server '{mcp_server_id}'. Cannot send schemas.")
			else:
				logger.warning(
					f"[{ws_stream_id}] No UI hierarchy generated by ProjectViewsService for '{mcp_server_id}'. Informing client.")
				await websocket.send_text(json.dumps(
					{"type": "status", "payload": {"message": f"Could not generate UI for server '{mcp_server_id}'."}}))
			...
			try:
				await websocket.send_text(
					json.dumps({"type": "error", "payload": {"message": "Failed to generate initial UI/send schemas"}}))
			except Exception as send_err:
				logger.error(f"[WS/{ws_stream_id}] Error sending UI gen error: {send_err}")
			disconnect_reason = "UI Generation Error"
			raise WebSocketDisconnect(code=1011, reason=disconnect_reason)
		...
		while True:
			...
			if 'WebSocketLogger' in locals() and hasattr(WebSocketLogger, 'log_text_received'):
				...
				logger.debug(f"[WS/{ws_stream_id}] RECV TEXT: {message_text[:100]}...")
			...
			if message_type == "ui_action":
				...
				try:
					...
					logger.info(
						f"[WS/{ws_stream_id}] DIAGNOSTIC - Extracted arguments for tool '{action_id}': {arguments_to_pass!r}")
					logger.info(
						f"[WS/{ws_stream_id}] Received ui_action '{action_id}' from element '{source_element_id}'")
			...
			# --- Handle notify_roots_changed ---
				...
				try:
					...
					if target_server_id == mcp_server_id:
						logger.info(
							f"[WS/{ws_stream_id}] Relaying roots_changed to MCP server '{target_server_id}'. Roots count: {len(payload.roots)}")
						if mcp_conn_manager:
							await mcp_conn_manager.notify_roots_changed(target_server_id, payload.roots)
						else:
							logger.error(f"[WS/{ws_stream_id}] MCPConnectionManager not available.")
					else:
						logger.warning(
							f"[WS/{ws_stream_id}] Received notify_roots_changed for wrong server ID (Expected: {mcp_server_id}, Got: {target_server_id}). Ignoring.")
				except ValidationError as e:
					logger.warning(f"[WS/{ws_stream_id}] Invalid 'notify_roots_changed' format: {e}")
					await websocket.send_text(
						json.dumps({"type": "error", "payload": {"message": f"Invalid roots changed format: {e}"}}))
				except Exception as e:
					logger.error(f"[WS/{ws_stream_id}] Error processing notify_roots_changed: {e}", exc_info=True)
					await websocket.send_text(json.dumps(
						{"type": "error", "payload": {"message": "Error processing roots changed notification"}}))
			...
			# --- Handle notify_cancelled ---
				...
				try:
					...
					if target_server_id == mcp_server_id:
						logger.info(
							f"[WS/{ws_stream_id}] Relaying cancellation for request '{request_id}' to MCP server '{target_server_id}'.")
						if mcp_conn_manager:
							await mcp_conn_manager.notify_cancelled(target_server_id, request_id)
						else:
							logger.error(f"[WS/{ws_stream_id}] MCPConnectionManager not available.")
					else:
						logger.warning(
							f"[WS/{ws_stream_id}] Received notify_cancelled for wrong server ID (Expected: {mcp_server_id}, Got: {target_server_id}). Ignoring.")
				except ValidationError as e:
					logger.warning(f"[WS/{ws_stream_id}] Invalid 'notify_cancelled' format: {e}")
					await websocket.send_text(
						json.dumps({"type": "error", "payload": {"message": f"Invalid cancel request format: {e}"}}))
				except Exception as e:
					logger.error(f"[WS/{ws_stream_id}] Error processing notify_cancelled: {e}", exc_info=True)
					await websocket.send_text(
						json.dumps({"type": "error", "payload": {"message": "Error processing cancel request"}}))

			# --- Existing ping/unknown handling ---
	...
	# --- Disconnect / Error Handling ---
		...
		if websocket.client_state == WebSocketState.CONNECTED:
			try:
				...
				logger.warning(f"[WS/{ws_stream_id}] Error closing WebSocket after exception: {close_err}")
	finally:
		# --- Cleanup Resources ---
		logger.info(
			f"[WS/{ws_stream_id}] Cleaning up WebSocket connection for user '{user_id}' (Reason: {disconnect_reason})...")
		if 'connection_manager' in locals() and connection_manager:
			...
			logger.warning(f"[WS/{ws_stream_id}] ConnectionManager unavailable during cleanup.")

		if session_established and session_key_for_cleanup:
			if 'mcp_conn_manager' in locals() and mcp_conn_manager:
				logger.info(
					f"[WS/{ws_stream_id}] Releasing MCP session reference for server '{session_key_for_cleanup}'...")
				try:
					await mcp_conn_manager.release_session(session_key_for_cleanup)
				except Exception as release_err:
					logger.error(
						f"[WS/{ws_stream_id}] Error releasing MCP session ref for '{session_key_for_cleanup}': {release_err}",
						exc_info=True)
			else:
				logger.warning(f"[WS/{ws_stream_id}] MCPConnectionManager unavailable during cleanup.")
		elif session_key_for_cleanup:
			logger.debug(
				f"[WS/{ws_stream_id}] MCP session was not established for '{session_key_for_cleanup}', no release needed.")
