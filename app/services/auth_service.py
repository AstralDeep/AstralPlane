...
try:
    ...
    class Settings: SECRET_KEY="fallback"; ALGORITHM="HS256"; ACCESS_TOKEN_EXPIRE_MINUTES=1
    settings=Settings()
    ...
    def get_user_repository(): return UserRepository()
...
class AuthService:
    ...
    def validate_token(self, token: str, context: str = "Unknown") -> Dict[str, Any]:
        ...
        try:
            ...
            if not user_profile:
                logger.warning(f"({context}) Failed authentication: User profile not found in repository for user ID: {user_id}")
            ...
            try:
                ...
                if hasattr(user_profile, 'model_dump') and callable(user_profile.model_dump):
                    user_dict = user_profile.model_dump()
                     ...
                     logger.error(f"({context}) Unexpected type for user_profile: {type(user_profile)}. Cannot convert to dict.")
            ...
            logger.info(f"({context}) Token successfully validated for user: {user_id}, expires: {token_expiry.isoformat()}Z")
...
async def authenticate_websocket(websocket: WebSocket) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
    ...
    try:
        ...
        logger.error(f"WS Auth Failed ({client_info}): JWTError during validation: {str(e)}", exc_info=True) # Log traceback for JWT errors
        return False, None, f"Invalid token format or signature." # Keep error message generic
    except Exception as e:
        # Catch any other unexpected errors during the process
        logger.error(f"WS Auth Error ({client_info}): Unexpected exception in authenticate_websocket: {e}", exc_info=True)
...
async def get_user_id(current_user: Dict[str, Any] = Depends(get_current_user)) -> str:
    ...
    if not user_id:
        ...
        logger.error("User ID missing from validated token data in get_user_id. This indicates an issue in validate_token or user profile structure.")
        raise HTTPException(
