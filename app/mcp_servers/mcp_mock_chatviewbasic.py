...
def construct_ui_layout() -> Dict[str, Any]:
   logger.info(f"MCP Server '{SERVER_NAME}' (ID: '{SERVER_ID_FOR_BINDING}') Constructing UI Layout. Bindings will use SERVER_ID_FOR_BINDING: '{SERVER_ID_FOR_BINDING}'")
   ...
   main_content_area = {
      ...
      "children": [
         {"id": "chat-display", "type": "ChatViewBasic",
          "config": {"title": f"{SERVER_NAME} Chat", "autoScroll": True, "style": {"flexGrow": 1, "minHeight": "200px"}},
          ...
          "children": [
             {"id": input_field_id, "type": "InputField",
              "config": {"placeholder": "Type...", "label": "Message", "style": {"flexGrow": 1},
                         "enterKeyAction": {"isEnabled": True, "actionId": chat_action_id, "targetElementId": "send-button"}}},
             {"id": "send-button", "type": "Button", "actionId": chat_action_id,
              "config": {"label": "Send", "variant": "primary", "valueSourceElementIds": [input_field_id],
                         "frontendActions": [
                             {"type": "echoToView", "sourceElementId": input_field_id, "targetBinding": chat_display_binding, "role": "user"},
   ...
   logger.debug(f"MCP Server '{SERVER_NAME}' (ID: '{SERVER_ID_FOR_BINDING}') Generated UI Layout with log_binding: '{log_binding}' and chat_display_binding: '{chat_display_binding}'")
   return root_layout

@mcp.tool(name="chatbot_query",description="Receives user query, calls LLM mock, returns assistant response as TextContent.")
async def chatbot_query(query: str) -> List[types.TextContent]:
   ...
   await ctx.info(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Received query: '{query}'") # Using display name and ID in log for clarity
   ...
   await ctx.info(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Asking client (backend) via create_message with {len(history)} messages...")
   try:
      if not ctx.session:
         await ctx.error(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] MCP Session context is missing for create_message.")
      ...
      await ctx.debug(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] DEBUG: Raw llm_response from create_message: {type(llm_response)} - {llm_response!r}")
      ...
      if (hasattr(llm_response, 'content') and
         ...
         await ctx.debug(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Extracted text via CreateMessageResult-like structure.")
      else:
         await ctx.warning(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Unexpected llm_response format: {type(llm_response)}. Stringifying.")
         assistant_response_text = str(llm_response)
      await ctx.info(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Processed assistant response text: '{assistant_response_text[:100]}...'")
      return [types.TextContent(type="text", text=assistant_response_text)]
   except Exception as e:
      await ctx.error(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING}) Req:{short_req_id}] Error during chatbot_query: {e}", exc_info=True)
...
if __name__ == "__main__":
   logger.info(f"--- Starting FastMCP server '{SERVER_NAME}' (ID: '{SERVER_ID_FOR_BINDING}', Protocol Name: '{mcp.name}') ---")
   starlette_app = mcp.sse_app()
   if starlette_app is None:
      logger.critical(f"[{SERVER_NAME} (ID:{SERVER_ID_FOR_BINDING})] mcp.sse_app() returned None. Cannot start server.")
      exit(1)
