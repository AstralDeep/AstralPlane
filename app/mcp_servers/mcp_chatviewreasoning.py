...
def construct_real_reasoning_ui_layout() -> dict:
	...
	left_column = {
		...
		"config": {
			...
			"style": {"flex": "2 1 0%"}
			# Keep flex in config.style (use 'flex' shorthand or 'flexGrow: 2', 'flexShrink: 1', 'flexBasis: 0%')
		},
...
"""
	...
	try:
		...
		async for chunk in stream:
			...
			if content:
				...
				if chunk_buffer and (
					...
					logger.debug(
						f"{log_prefix} Batch ready (Size: {len(chunk_buffer)}). Putting batch onto queue for raw stream.")
		...
		if chunk_buffer:
			logger.debug(
				f"{log_prefix} Flushing final buffer content to queue for raw stream (Size: {len(chunk_buffer)}).")
			...
			if sender_task:
				...
				try:
					...
					try:
						...
						logger.debug(f"{log_prefix} FINALLY: Sender task cancellation confirmed.")
					except Exception as final_task_err:
						logger.error(f"{log_prefix} FINALLY: Error awaiting cancelled sender task: {final_task_err}")
				except Exception as e:
					logger.error(
						f"{log_prefix} FINALLY: Error during sender task final wait/cancel: {type(e).__name__}")
			else:
	...
	if llm_error_occurred:
		...
		match = re.search(r"<answer>(.*?)</answer>", accumulated_full_response,
						  re.DOTALL | re.IGNORECASE)  # DOTALL matches newlines
		if match:
			...
			logger.warning(
				f"{log_prefix} <answer> tags not found in the response. Returning full response as fallback.")
	...
	await ctx.info(
		f"[{completion_timestamp} Req: {short_req_id}] Reasoning task completed. Returning final result for chat view.")  # Standard log
...
if __name__ == "__main__":
	...
	logger.debug(" - Using Standard Logging Channel (ctx.session.send_log_message) for raw streaming.")  # Indicate the current approach
	...
	logger.debug(" - Final answer for chat view is filtered for <answer> tags.")  # Added note
	logger.debug(f" - In-memory history enabled, keeping last {MAX_HISTORY_TURNS} turns.")
	logger.debug(" - Styling configuration aligned with frontend feedback (using config/config.style).")  # Updated note
	logger.debug("----------------------------------------------------")
