...
try:
    ...
    print("Successfully imported MCP libraries.")
     ...
     class FastMCP:
         def __init__(self,*args,**kwargs): pass; tool=lambda *a,**k: lambda f:f; get_context=lambda s: Context(); sse_app=lambda s:None; settings=type('Settings',(),{'host':'localhost','port':8000,'log_level':'info'})()
     class types_dummy: TextContent=type('TextContent',(),{'__init__':lambda s,**k:setattr(s,'text',k.get('text'))}); SamplingMessage=type('SamplingMessage',(),{'__init__':lambda s,**k:None}) # Simplified dummies
     class Context: session=None; _request_context=None; request_id=f"dummy-req-{uuid.uuid4()}";
     async def log(s,*a,**k):pass;
     async def debug(s,*a,**k):pass;
     async def info(s,*a,**k):pass;
     async def warning(s,*a,**k):pass;
     async def error(s,*a,**k):pass
     class RequestContext: pass
     types = types_dummy # Assign dummies if import failed

# --- Configuration ---
HOST = "127.0.0.1"; PORT = 8123; SERVER_NAME = "mcp_mock_chatviewbasic"
...
# --- UI Structure Function (MODIFIED Layout) ---
def construct_ui_layout() -> dict:
    """ Constructs the UI layout dictionary with LogView positioned higher. """
    server_logger.info(f"[{SERVER_NAME}] Constructing UI Layout (LogView Higher)...")
    ...
    main_content_area = {
        ...
        "children": [
            {"id": "chat-display", "type": "ChatViewBasic",
             ...
             "config": {"title": f"{SERVER_NAME} Chat", "autoScroll": True, "style": {"flexGrow": 1, "minHeight": "200px"}}, # Added minHeight
             "updateBinding": chat_display_binding},
            {"id": "input-area", "type": "StackLayout",
             # Input area takes its natural height
             "config": {"direction": "horizontal", "gap": "8px", "align_items": "flex-end", "style": {"paddingTop": "10px"}},
             "children": [
                 {"id": input_field_id, "type": "InputField", "config": {"placeholder": "Type...", "label": "Message", "style": {"flexGrow": 1}, "enterKeyAction": {"isEnabled": True, "actionId": chat_action_id, "targetElementId": "send-button"}}},
                 {"id": "send-button", "type": "Button", "actionId": chat_action_id, "config": {"label": "Send", "variant": "primary", "valueSourceElementIds": [input_field_id], "frontendActions": [{"type": "echoToView", "sourceElementId": input_field_id, "targetBinding": chat_display_binding, "role": "user"}, {"type": "clearElement", "targetElementId": input_field_id}]}}]}
        ]
    }
    ...
    log_view_area = {
        ...
        "config": {"title": "Server Logs", "lineFormat": "json", "autoScroll": True, "style": {"height": "200px", "marginTop": "16px"}}, # Fixed height log view
...
async def chatbot_query(query: str) -> List[types.TextContent]: # Return TextContent
    ...
    if not history: session_histories[request_key] = history; await ctx.info(f"[{SERVER_NAME} Req:{short_req_id}] Initialized new history.")
    ...
    await ctx.info(f"[{SERVER_NAME} Req:{short_req_id}] Asking client via create_message with {len(history)} messages...")
    ...
    try:
        ...
        if (hasattr(llm_response, 'content') and hasattr(llm_response, 'role') and
            ...
            hasattr(llm_response.content, 'type') and getattr(llm_response.content, 'type') == 'text'):
             ...
             await ctx.debug(f"[{SERVER_NAME} Req:{short_req_id}] Extracted text via CreateMessageResult-like structure.")
             ...
             await ctx.warning(f"[{SERVER_NAME} Req:{short_req_id}] Extracted text directly from TextContent-like response.")
        else:
             await ctx.error(f"[{SERVER_NAME} Req:{short_req_id}] Unexpected llm_response format: {type(llm_response)}. Stringifying.")
             assistant_response_text = str(llm_response)

        await ctx.debug(f"[{SERVER_NAME} Req:{short_req_id}] DEBUG: Final assistant_response_text: {assistant_response_text!r}")
        await ctx.info(f"[{SERVER_NAME} Req:{short_req_id}] Processed assistant response text: '{assistant_response_text[:100]}...'")
...
async def get_ui_layout() -> List[Any]:
    ctx = mcp.get_context(); await ctx.info(f"[{SERVER_NAME} Tool:get_ui_layout] Called.")
    ui_layout_dict = construct_ui_layout()
    try: ui_layout_json = json.dumps(ui_layout_dict); return [types.TextContent(type="text", text=ui_layout_json)]
    except TypeError as json_err: await ctx.error(f"Failed layout: {json_err}"); return [types.TextContent(type="text", text=f"Error: {json_err}")]

# --- Main execution block --- (Unchanged)
if __name__ == "__main__":
    # (Startup logic unchanged)
    print(f"--- Starting FastMCP server ({SERVER_NAME}) ---")
    starlette_app = mcp.sse_app()
    if starlette_app is None: print(f"--- CRITICAL ERROR: mcp.sse_app() returned None. ---"); exit(1)
    run_host = mcp.settings.host if hasattr(mcp, 'settings') else HOST
    run_port = mcp.settings.port if hasattr(mcp, 'settings') else PORT
    log_level = mcp.settings.log_level.lower() if hasattr(mcp, 'settings') and hasattr(mcp.settings, 'log_level') else "debug"
    print(f"Listen on: http://{run_host}:{run_port} with log level {log_level}")
    try: uvicorn.run(starlette_app, host=run_host, port=run_port, log_level=log_level)
    except Exception as e: server_logger.critical(f"Failed Uvicorn: {e}", exc_info=True); exit(1)