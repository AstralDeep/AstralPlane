# app/mcp_mock_chatviewreasoning.py (Layout Updated - No Cancellation)
...
# --- Assume MCP libraries are available ---
# (Using dummies from previous example if not installed)
try:
    ...
    print("Successfully imported MCP libraries.")
     ...
     class FastMCP:
         ...
         def get_context(self) -> 'Context': return Context()
         def sse_app(self): return None
         settings = type('Settings', (), {'host':'localhost', 'port':8000, 'log_level':'info'})()
     class types:
          class TextContent:
              def __init__(self, *, type: str, text: str): self.type = type; self.text = text
     class Context:
         session = None; _request_context = None; request_id = f"dummy-req-{uuid.uuid4()}"
         async def log(self, *args, **kwargs): print(f"DUMMY_LOG: {' '.join(map(str, args))}")
         async def debug(self, *args, **kwargs): print(f"DUMMY_DEBUG: {' '.join(map(str, args))}")
         async def info(self, *args, **kwargs): print(f"DUMMY_INFO: {' '.join(map(str, args))}")
         async def warning(self, *args, **kwargs): print(f"DUMMY_WARN: {' '.join(map(str, args))}")
         async def error(self, *args, **kwargs): print(f"DUMMY_ERROR: {' '.join(map(str, args))}")
...
HOST = "127.0.0.1"
PORT = 8125
SERVER_NAME = "mcp_mock_chatviewreasoning" # Keep corrected name
...
def construct_reasoning_ui_layout() -> dict:
    ...
    server_logger.info(f"[{SERVER_NAME}] Constructing Updated 2/3 + 1/3 Layout...")
    ...
    left_column = {
        ...
        "children": [
            ...
            {"id": "chat-display", "type": "ChatViewBasic",
             "config": {"title": "Chat Conversation", "autoScroll": True, "style": {"flexGrow": 1, "minHeight": "100px"}}, # Let it grow
    ...
    return {
        ...
        "children": [
            {"id": "title", "type": "TextView", "config": {"initialText": "MCP Reasoning Chat Demo", "variant":"headline"}},
            {"id": "explanation", "type": "TextView", "config": {"initialText": "Enter a query. Assistant shows reasoning (right) and final answer (left).", "variant":"body"}}, # Simplified explanation
...
if __name__ == "__main__":
    print(f"--- Starting FastMCP Reasoning Chat server ({SERVER_NAME}) ---")
    # ... (rest of the main block is unchanged) ...
    if not MCP_AVAILABLE:
         from starlette.applications import Starlette
         from starlette.responses import PlainTextResponse
         from starlette.routing import Route
         async def homepage(request): return PlainTextResponse('Dummy Server Running')
         starlette_app = Starlette(routes=[Route("/", homepage)])
         print("WARNING: Running with dummy Starlette app as MCP libs are missing.")
    else:
        starlette_app = mcp_server.sse_app()

    if starlette_app is None:
         server_logger.critical(f"CRITICAL ERROR: Failed to get Starlette app from MCP server. Cannot start.")
         exit(1)
    ...
    print("This server demonstrates streaming reasoning logs (Updated Layout).")
    print("----------------------------------------------------")

    try:
        uvicorn.run(starlette_app, host=run_host, port=run_port, log_level=log_level)
    except Exception as e:
         server_logger.critical(f"Failed to run Uvicorn server: {e}", exc_info=False)
         exit(1)